...
mc_vars <- all_output %>% 
  ...
  pivot_wider(names_from = augmented, values_from = c(mc_var, bias),
              names_prefix = "aug_") %>% 
  mutate(relative_efficiency = mc_var_aug_FALSE / mc_var_aug_TRUE,
...
continuous_bias_plot <- bias_tib %>% 
  filter(outcome == "IC80") %>% 
  ggplot(aes(x = pred_perf, y = bias, color = bnab, shape = augmented)) +
  geom_point(size = point_size) +
  ...
  scale_x_continuous(sec.axis = sec_axis(~ ., name = "Percentage Increase in Available Viruses",
                                         breaks = NULL, labels = NULL)) +
  scale_y_continuous(labels = function(x) sprintf("%g", x), limits = c(-1e-3, 1e-3)) +
  facet_grid(rows = vars(outcome), cols = vars(percentage), labeller = label_both)

binary_bias_plot <- bias_tib %>% 
  filter(outcome == "IC80 < 1") %>% 
  ggplot(aes(x = pred_perf, y = bias, color = bnab, shape = augmented)) +
  geom_point(size = point_size) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
